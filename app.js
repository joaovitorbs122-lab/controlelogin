...
function simItemHTML(s, store) {
  ...
  // Discursiva mini badges (apenas para Simulados Gerais)
  let discBadges = '';
  if (store === 'simulados' && s.discursiva && s.discursiva.length) {
    const totalDisc = s.discursiva.reduce((sum, mat) =>
      sum + (mat.questoes||[]).reduce((s2,q) => s2 + (q.nota||0), 0), 0);
    const matBadges = s.discursiva.map(mat => {
    ...
    discBadges = `<div class="sim-materias-mini" style="margin-top:4px;border-top:1px dashed var(--border);padding-top:4px">
      ${matBadges}
      <span class="disc-nota-badge" style="font-size:11px">Total disc.: ${totalDisc.toFixed(2)} pts</span>
    </div>`;
  }
...
function addSimDiscursivaMateria(wrapId, dadosMateria = null) {
  ...
  const uniqId = 'disc-mat-dl-' + Math.random().toString(36).slice(2);
  block.innerHTML = `
    <datalist id="${uniqId}">
      ${MATERIAS.map(m => `<option value="${m}"></option>`).join('')}
    </datalist>
    <div class="disc-materia-header">
      <span class="disc-materia-label">Disciplina:</span>
      <input type="text" class="disc-sel-materia" list="${uniqId}" placeholder="Ex: Biologia, Química…" value="${materiaVal}" style="min-width:140px" />
      <span class="disc-materia-label" style="margin-left:8px">Questões:</span>
      <input type="number" class="disc-inp-nq" min="1" max="30" value="${numQuestoes}" style="width:60px" />
      <span class="disc-materia-label" style="margin-left:8px">Itens/questão:</span>
      <input type="number" class="disc-inp-ni" min="1" max="10" value="${numItens}" style="width:60px" />
...
async function renderDiscursivos() {
  ...
  const sims  = await dbGetAll('simulados_discursivos');
  if (!sims.length) {
...
async function openModalDiscursivo(id) {
  const s = await dbGet('simulados_discursivos', id);
  if (!s) return;
  const totalNota = (s.disciplinas||[]).reduce((sum, mat) =>
    sum + (mat.questoes||[]).reduce((s2,q) => s2 + (q.nota||0), 0), 0);
  let html = `
