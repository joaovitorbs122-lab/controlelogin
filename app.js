...
function navigateTo(pageId) {
  ...
  const renders = {
    ...
    simulados_discursivos:  renderSimuladosDiscursivos,
  };
  renders[pageId]?.();
}
...
function initSimulados() {
  ...
  // Discursivas inline no form de simulados gerais
  document.getElementById('btnAddSimDiscursiva').addEventListener('click', () => {
    const mat   = document.getElementById('simDiscursivaMateria').value;
    const qtd   = parseInt(document.getElementById('simDiscursivaQtd').value);
    const itens = parseInt(document.getElementById('simDiscursivaItens').value);
    if (!mat)        { showToast('Selecione a matéria.', 'error'); return; }
    if (!qtd || qtd < 1) { showToast('Informe o número de questões.', 'error'); return; }
    if (!itens || itens < 1) { showToast('Informe os itens por questão.', 'error'); return; }
    appendSimDiscursivaBlock(mat, qtd, itens);
    document.getElementById('simDiscursivaMateria').value = '';
    document.getElementById('simDiscursivaQtd').value = '';
    document.getElementById('simDiscursivaItens').value = '';
  });

  document.getElementById('formSimulado').addEventListener('submit', async e => {
    e.preventDefault();
    const materias = readMateriasTable('simMateriasBody');
    const discursivas = readSimDiscursivas();
    if (!materias.length && !discursivas.length) { showToast('Preencha pelo menos uma matéria ou discursiva.', 'error'); return; }
    ...
    const obj = {
      ...
      materias, discursivas, tipo: 'geral',
    };
    if (id) obj.id = id;
...
/* Helper: adiciona bloco de discursiva inline no form de simulados gerais */
function appendSimDiscursivaBlock(mat, qtdQuestoes, itensQuestao, valores = []) {
  const wrap = document.getElementById('simDiscursivasWrap');
  const blockId = 'simDisc_' + mat.replace(/\s/g,'') + '_' + Date.now();
  const letras = 'abcdefghij'.split('');

  let questoesHtml = '';
  for (let q = 0; q < qtdQuestoes; q++) {
    const qVal = valores[q] || {};
    let itensHtml = '';
    for (let it = 0; it < itensQuestao; it++) {
      const itemVal = (qVal.itens || [])[it] ?? '';
      itensHtml += `<div class="disc-item-row">
        <span class="disc-item-label">${letras[it]})</span>
        <input type="number" class="disc-item-input" min="0" step="0.1" placeholder="0" value="${itemVal}" data-q="${q}" data-it="${it}" />
      </div>`;
    }
    const qNota = qVal.nota ?? '';
    questoesHtml += `<div class="disc-questao-block">
      <div class="disc-questao-header">
        <span class="disc-q-label">Questão ${q+1}</span>
        <span class="disc-q-total">Total: <strong class="disc-q-total-val" data-q="${q}">—</strong></span>
      </div>
      <div class="disc-itens-wrap">${itensHtml}</div>
    </div>`;
  }

  const div = document.createElement('div');
  div.className = 'disc-materia-block';
  div.dataset.mat = mat;
  div.dataset.qtd = qtdQuestoes;
  div.dataset.itens = itensQuestao;
  div.id = blockId;
  div.innerHTML = `
    <div class="disc-materia-header">
      <span class="disc-materia-name">${mat}</span>
      <span class="disc-materia-summary">${qtdQuestoes} questão(ões) · ${itensQuestao} item(ns) cada</span>
      <button type="button" class="btn-icon disc-remove-btn" data-block="${blockId}">✕</button>
    </div>
    <div class="disc-questoes-wrap">${questoesHtml}</div>
    <div class="disc-materia-total">Total da disciplina: <strong class="disc-mat-total-val">—</strong></div>`;
  wrap.appendChild(div);

  // listeners para somar itens → questão → total matéria
  div.querySelectorAll('.disc-item-input').forEach(inp => {
    inp.addEventListener('input', () => updateSimDiscTotals(div));
  });
  div.querySelector('.disc-remove-btn').addEventListener('click', () => div.remove());
  updateSimDiscTotals(div);
}

function updateSimDiscTotals(blockEl) {
  const questoesMap = {};
  blockEl.querySelectorAll('.disc-item-input').forEach(inp => {
    const q = inp.dataset.q;
    if (!questoesMap[q]) questoesMap[q] = 0;
    questoesMap[q] += parseFloat(inp.value) || 0;
  });
  let matTotal = 0;
  Object.entries(questoesMap).forEach(([q, soma]) => {
    const el = blockEl.querySelector(`.disc-q-total-val[data-q="${q}"]`);
    if (el) el.textContent = soma.toFixed(2);
    matTotal += soma;
  });
  const matTotEl = blockEl.querySelector('.disc-mat-total-val');
  if (matTotEl) matTotEl.textContent = matTotal.toFixed(2);
}

function readSimDiscursivas() {
  const wrap = document.getElementById('simDiscursivasWrap');
  if (!wrap) return [];
  const result = [];
  wrap.querySelectorAll('.disc-materia-block').forEach(block => {
    const mat = block.dataset.mat;
    const qtd = parseInt(block.dataset.qtd);
    const itensQuestao = parseInt(block.dataset.itens);
    const questoes = [];
    for (let q = 0; q < qtd; q++) {
      const itens = [];
      for (let it = 0; it < itensQuestao; it++) {
        const inp = block.querySelector(`.disc-item-input[data-q="${q}"][data-it="${it}"]`);
        itens.push(parseFloat(inp?.value) || 0);
      }
      const nota = itens.reduce((s, v) => s + v, 0);
      questoes.push({ itens, nota });
    }
    const totalMateria = questoes.reduce((s, q) => s + q.nota, 0);
    result.push({ materia: mat, questoes, totalMateria, itensQuestao });
  });
  return result;
}
...
function openFormSimulado(dados = null) {
  ...
  // Restore discursivas
  const discWrap = document.getElementById('simDiscursivasWrap');
  if (discWrap) {
    discWrap.innerHTML = '';
    (dados?.discursivas || []).forEach(d => {
      appendSimDiscursivaBlock(d.materia, d.questoes.length, d.itensQuestao, d.questoes);
    });
  }
...
async function openModalSimulado(id, store) {
  ...
  let discursivasHtml = '';
  if ((s.discursivas||[]).length) {
    discursivasHtml = `<div class="modal-section-title" style="margin-top:20px">Notas Discursivas</div>`;
    s.discursivas.forEach(d => {
      discursivasHtml += `<div style="margin-bottom:14px">
        <div style="font-weight:600;font-size:14px;margin-bottom:6px;color:var(--accent)">${d.materia} — Total: ${(d.totalMateria||0).toFixed(2)}</div>
        <table class="modal-mat-table">
          <thead><tr><th>Questão</th><th>Itens</th><th>Nota</th></tr></thead>
          <tbody>
            ${(d.questoes||[]).map((q,qi) => {
              const letras = 'abcdefghij'.split('');
              const itensStr = (q.itens||[]).map((v,i) => `${letras[i]}) ${v.toFixed(2)}`).join(' · ');
              return `<tr><td>Q${qi+1}</td><td style="font-size:12px;color:var(--text-secondary)">${itensStr}</td><td><strong>${(q.nota||0).toFixed(2)}</strong></td></tr>`;
            }).join('')}
          </tbody>
        </table>
      </div>`;
  ...
  const html = `
    ...
    ${totalTot ? `<div class="modal-row"><span class="modal-row-label">Total Objetiva</span><span class="modal-row-value pct-badge ${pctClass(p)}">${p}% (${totalAc}/${totalTot})</span></div>` : ''}
    ${totalTot ? `<div class="modal-section-title">Desempenho por Matéria (Objetiva)</div>
    <table class="modal-mat-table">
...
const LETRAS_ITENS = 'abcdefghij'.split('');

function initSimuladosDiscursivos() {
  ...
  document.getElementById('modalDiscursivo').addEventListener('click', e => {
    if (e.target === document.getElementById('modalDiscursivo')) document.getElementById('modalDiscursivo').classList.add('hidden');
  });

  document.getElementById('btnAddDiscMateria').addEventListener('click', () => {
    const mat   = document.getElementById('discMateriaAdd').value;
    const qtd   = parseInt(document.getElementById('discQtdQuestoes').value);
    const itens = parseInt(document.getElementById('discQtdItens').value);
    if (!mat)        { showToast('Selecione a disciplina.', 'error'); return; }
    if (!qtd || qtd < 1)   { showToast('Informe o número de questões.', 'error'); return; }
    if (!itens || itens < 1) { showToast('Informe os itens por questão.', 'error'); return; }
    appendDiscDisciplinaBlock(mat, qtd, itens);
    document.getElementById('discMateriaAdd').value = '';
    document.getElementById('discQtdQuestoes').value = '';
    document.getElementById('discQtdItens').value = '';
  });

  document.getElementById('formDiscursivo').addEventListener('submit', async e => {
    e.preventDefault();
    const disciplinas = readDiscDisciplinas();
    ...
    const obj = {
      ...
      disciplinas, tipo: 'discursivo',
    };
    if (id) obj.id = id;
    await dbSave('simulados_discursivos', obj);
    showToast('Prova discursiva salva!', 'success');
    document.getElementById('formDiscursivoCard').classList.add('hidden');
    renderSimuladosDiscursivos();
  });
}

function openFormDiscursivo(dados = null) {
  document.getElementById('formDiscursivoTitle').textContent = dados ? 'Editar Prova Discursiva' : 'Nova Prova Discursiva';
  document.getElementById('discId').value = dados?.id || '';
  ...
  const wrap = document.getElementById('discDisciplinasWrap');
  wrap.innerHTML = '';
  (dados?.disciplinas || []).forEach(d => {
    appendDiscDisciplinaBlock(d.materia, d.questoes.length, d.itensQuestao, d.questoes);
  });
...
function appendDiscDisciplinaBlock(mat, qtdQuestoes, itensQuestao, valores = []) {
  const wrap   = document.getElementById('discDisciplinasWrap');
  const blockId = 'disc_' + mat.replace(/\s/g,'') + '_' + Date.now();
  ...
  for (let q = 0; q < qtdQuestoes; q++) {
    ...
    for (let it = 0; it < itensQuestao; it++) {
      const itemVal = (qVal.itens || [])[it] ?? '';
      itensHtml += `<div class="disc-item-row">
        <span class="disc-item-label">${LETRAS_ITENS[it]})</span>
        <input type="number" class="disc-item-input" min="0" step="0.1" placeholder="0" value="${itemVal}" data-q="${q}" data-it="${it}" />
      </div>`;
    }
    questoesHtml += `<div class="disc-questao-block">
      <div class="disc-questao-header">
        <span class="disc-q-label">Questão ${q+1}</span>
        <span class="disc-q-total">Nota: <strong class="disc-q-total-val" data-q="${q}">0,00</strong></span>
      </div>
      <div class="disc-itens-wrap">${itensHtml}</div>
    </div>`;
  }
  ...
  div.innerHTML = `
    <div class="disc-materia-header">
      <span class="disc-materia-name">${mat}</span>
      <span class="disc-materia-summary">${qtdQuestoes} questão(ões) · ${itensQuestao} item(ns)/questão</span>
      <button type="button" class="btn-icon disc-remove-btn" data-block="${blockId}">✕</button>
    </div>
    <div class="disc-questoes-grid">${questoesHtml}</div>
    <div class="disc-materia-total">
      <span>Total da disciplina:</span>
      <strong class="disc-mat-total-val">0,00</strong>
    </div>`;
  wrap.appendChild(div);

  div.querySelectorAll('.disc-item-input').forEach(inp => {
    inp.addEventListener('input', () => updateDiscTotals(div));
  });
  div.querySelector('.disc-remove-btn').addEventListener('click', () => div.remove());
  if (valores.length) updateDiscTotals(div);
}

function updateDiscTotals(blockEl) {
  const questoesMap = {};
  blockEl.querySelectorAll('.disc-item-input').forEach(inp => {
    const q = inp.dataset.q;
    if (!questoesMap[q]) questoesMap[q] = 0;
  ...
  Object.entries(questoesMap).forEach(([q, soma]) => {
    const el = blockEl.querySelector(`.disc-q-total-val[data-q="${q}"]`);
    if (el) el.textContent = soma.toFixed(2).replace('.',',');
    matTotal += soma;
  });
  const matTotEl = blockEl.querySelector('.disc-mat-total-val');
  if (matTotEl) matTotEl.textContent = matTotal.toFixed(2).replace('.',',');
}

function readDiscDisciplinas() {
  const wrap = document.getElementById('discDisciplinasWrap');
  if (!wrap) return [];
  const result = [];
  wrap.querySelectorAll('.disc-materia-block').forEach(block => {
...
async function renderSimuladosDiscursivos() {
  const provas = await dbGetAll('simulados_discursivos');
  const lista  = document.getElementById('listaDiscursivos');
  if (!provas.length) {
    lista.innerHTML = '<p class="empty-state" style="padding:32px 0">Nenhuma prova discursiva cadastrada.</p>';
    return;
  }
  const sorted = [...provas].sort((a, b) => (b.data||'').localeCompare(a.data||''));
  lista.innerHTML = sorted.map(p => discursivoItemHTML(p)).join('');
  lista.querySelectorAll('.sim-item').forEach(el => {
    el.addEventListener('click', () => openModalDiscursivo(el.dataset.id));
  });
}

function discursivoItemHTML(p) {
  const totalGeral = (p.disciplinas||[]).reduce((s, d) => s + (d.totalMateria||0), 0);
  const miniBadges = (p.disciplinas||[]).map(d =>
    `<span class="mini-badge mid">${d.materia}: ${(d.totalMateria||0).toFixed(1)}</span>`
  ).join('');
  return `
  <div class="sim-item" data-id="${p.id}">
    <div class="sim-info" style="width:100%">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
        <div>
          <span class="sim-name">${p.nome||'Prova'}</span>
          <span class="sim-meta" style="display:block">${fmtDate(p.data)} · ${(p.disciplinas||[]).length} disciplina(s)</span>
        </div>
        <div class="sim-stats">
          <span class="diff-badge diff-${p.dificuldade||'medio'}">${p.dificuldade||'médio'}</span>
          <span class="pct-badge mid" style="font-size:15px">${totalGeral.toFixed(1)} pts</span>
          <div class="sim-actions" onclick="event.stopPropagation()">
            <button class="btn-icon" onclick="editDiscursivo('${p.id}')">✎</button>
            <button class="btn-danger" onclick="deleteDiscursivo('${p.id}')">✕</button>
          </div>
        </div>
      </div>
      <div class="sim-materias-mini">${miniBadges}</div>
...
async function openModalDiscursivo(id) {
  const p = await dbGet('simulados_discursivos', id);
  const totalGeral = (p.disciplinas||[]).reduce((s, d) => s + (d.totalMateria||0), 0);

  let discHtml = '';
  (p.disciplinas||[]).forEach(d => {
    discHtml += `<div style="margin-bottom:18px">
      <div style="font-weight:600;font-size:14px;margin-bottom:6px;color:var(--accent)">${d.materia} — Total: ${(d.totalMateria||0).toFixed(2)}</div>
      <table class="modal-mat-table">
        <thead><tr><th>Questão</th><th>Itens</th><th>Nota</th></tr></thead>
        <tbody>
          ${(d.questoes||[]).map((q,qi) => {
            const itensStr = (q.itens||[]).map((v,i) => `${LETRAS_ITENS[i]}) ${v.toFixed(2)}`).join(' · ');
            return `<tr><td>Q${qi+1}</td><td style="font-size:12px;color:var(--text-secondary)">${itensStr}</td><td><strong>${(q.nota||0).toFixed(2)}</strong></td></tr>`;
          }).join('')}
        </tbody>
      </table>
  ...
  const html = `
    <div class="modal-row"><span class="modal-row-label">Nome</span><span class="modal-row-value">${p.nome}</span></div>
    <div class="modal-row"><span class="modal-row-label">Data</span><span class="modal-row-value">${fmtDate(p.data)}</span></div>
    <div class="modal-row"><span class="modal-row-label">Dificuldade</span><span class="modal-row-value"><span class="diff-badge diff-${p.dificuldade}">${p.dificuldade}</span></span></div>
    <div class="modal-row"><span class="modal-row-label">Total Geral</span><span class="modal-row-value"><strong>${totalGeral.toFixed(2)}</strong> pontos</span></div>
    <div class="modal-section-title" style="margin-top:16px">Detalhamento por Disciplina</div>
    ${discHtml}`;

  document.getElementById('modalDiscursivoTitle').textContent = p.nome;
...
window.editDiscursivo = async function(id) { openFormDiscursivo(await dbGet('simulados_discursivos', id)); };
window.deleteDiscursivo = async function(id) {
  if (!confirm('Remover esta prova discursiva?')) return;
  await dbDelete('simulados_discursivos', id);
