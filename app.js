...
async function renderSimulados() {
  ...
  lista.querySelectorAll('.sim-item').forEach(el => {
    el.addEventListener('click', () => openModalSimulado(parseInt(el.dataset.id), 'simulados'));
  });
}
...
async function openModalSimulado(id, store) {
  ...
  const html = `
    ...
    <div class="modal-section-title">Desempenho por Matéria</div>
...
async function renderEnem() {
  ...
  lista.querySelectorAll('.sim-item').forEach(el => {
    el.addEventListener('click', () => openModalSimulado(parseInt(el.dataset.id), 'enem'));
  });
}
...
async function renderFuvest() {
  ...
  lista.querySelectorAll('.sim-item').forEach(el => {
    el.addEventListener('click', () => openModalSimulado(parseInt(el.dataset.id), 'fuvest'));
  });
}
...
function renderPendentes(revs, el) {
  ...
  el.querySelectorAll('.rev-check').forEach(cb => {
    cb.addEventListener('change', async () => {
      ...
      const revId = parseInt(cb.dataset.revId), revIdx = parseInt(cb.dataset.revIdx);
...
function initErros() {
  ...
  document.getElementById('formErro').addEventListener('submit', async e => {
    ...
    if (id && !fileE) { const old = await dbGet('erros', parseInt(id)); imgEnunciado = old?.imgEnunciado || null; }
    if (id && !fileR) { const old = await dbGet('erros', parseInt(id)); imgResolucao = old?.imgResolucao || null; }
...
async function renderErros() {
  ...
  lista.querySelectorAll('.erro-card').forEach(el => {
    el.addEventListener('click', () => openModalErro(parseInt(el.dataset.id)));
  });
}
